...
const Toggle = ({ name, value, onChange = () => {} }) => {
    return (
        <label className="relative inline-flex items-center cursor-pointer w-fit">
            <input onChange={onChange} value={value} type="checkbox" className="sr-only peer" />
            ...
            {name && <span className="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300">{name}</span>}
        </label>
    );
};
