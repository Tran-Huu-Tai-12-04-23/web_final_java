...
function Header() {
        ...
        return NavHeader.map((item, index) => {
            return (
                <Link
                    to={item.path}
                    key={index}
                    className="text-light dark:text-dark hover:text-hover dark:hover:text-hover cursor-pointer rounded-md px-3 py-2 text-sm font-thin"
                >
                    {item.name}
                </Link>
            );
    ...
    return (
        <Main
            className={
                ' flex items-center justify-between border-header border-b-[1px] border-solid fixed top-0 left-0 right-0'
            }
        >
            <Link to="/">
